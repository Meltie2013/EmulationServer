...
//
// File: src/WorldServer/Program.cs
// Purpose: Contains program code for the world server gameplay, session, and character runtime layer.
// Documentation: Uses normal line comments so the source stays readable without C# XML documentation tags.

using EmulationServer.WorldServer.Configuration;
using EmulationServer.WorldServer.Core;
...
try
{
    string configurationPath = args.Length > 0 ? args[0] : Path.Combine(AppContext.BaseDirectory, "worldserver.ini");

    WorldServerSettings settings = WorldServerConfigurationLoader.Load(configurationPath);
...
{
    Logger.Write(LogType.INFORMATION, "Shutdown requested. Stopping WorldServer...", "Program");
}

...
using EmulationServer.CharacterServer.Core;
...
Logger.Write(LogType.NOTICE, @"                          :: Character Server ::");
...
try
{
    string configurationPath = args.Length > 0 ? args[0] : Path.Combine(AppContext.BaseDirectory, "characterserver.ini");

    CharacterServerSettings settings = CharacterServerConfigurationLoader.Load(configurationPath);

    await using CharacterServer server = new(settings);

    await server.StartAsync(cancellation.Token);
}
catch (OperationCanceledException) when (cancellation.IsCancellationRequested)
{
    Logger.Write(LogType.INFORMATION, "Shutdown requested. Stopping CharacterServer...", nameof(Program));
}

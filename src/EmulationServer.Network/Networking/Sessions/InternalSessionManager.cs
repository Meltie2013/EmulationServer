...
//
// File: src/EmulationServer.Network/Networking/Sessions/InternalSessionManager.cs
// Purpose: Contains internal session manager code for the packet serialization, socket transport, and protocol framing layer.
...
// Purpose: Provides internal session manager behavior for the packet serialization, socket transport, and protocol framing layer.
...
public sealed class InternalSessionManager
{
    ...
    // Notes: This keeps the operation scoped to InternalSessionManager so callers do not duplicate validation, protocol, or persistence rules.
    public bool TryAddSession(InternalServerSession session)
    {
    ...
    public async Task WaitForAllSessionsAsync(TimeSpan timeout, CancellationToken cancellationToken = default)
    {
        ...
        {
            Logger.Write(LogType.WARNING, "Stopped waiting for internal sessions because shutdown wait was cancelled.", "InternalSessionManager");
            return;
        }

        Logger.Write(LogType.WARNING, $"Stopped waiting for internal sessions because shutdown wait timed out after {timeout.TotalSeconds:0.##} second(s).",
    ...
    private sealed class SessionEntry
    {
        ...
        public InternalServerSession Session { get; }
